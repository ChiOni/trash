...
<br/>
<center><img src="/assets/img/pe/gnn/gnnthree.jpg" width="50%" height="50%"></center>
<br/>
...
```
<br/>
GNN 모델의 구현으로 바로 넘어가보자.  
  
GNN 모델은 2번의 GNN_layer를 넘어가게 되는데 GNN_layer함수가 의미하는 바가 무엇인지 이해해야 한다.  
  
GNN_layer class의 forward 함수는 A와 X의 곱으로 얻어진 매트릭스를 row 단위로 fully connected layer로 넘기는 역할을 한다.  
...
방금 생성한 GNN 모델과 비교의 대상이 될 base FC 모델을 같은 방법으로 학습할 것이다.  
...
# GNNN 학습 돌려서 epoch에 따른 Loss 확인
model = GNN(features.size(1) , labels.unique().size(0), A)
criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(model.parameters(), lr=0.1, weight_decay=0.0001)
...
```
<br/>
FCN을 통한 최종 ACC는 0.5770 GNN을 통한 최종 ACC는 0.8280으로 나타났다.  
  
사실 직관적으로 너무 당연하게도 GNN의 성능이 더 우수할 수 밖에 없다.  
  
FCN이 분류에 활용할 수 있는 정보는 해당 논문의 단어 데이터 밖에 없는데 반해 GNN은 연결된 다른 논문의 정보도 활용하기 때문이다.  
...
이어서 응용 버젼인 Graph Attention Network와 Graph Convolution Network를 실습하고 최신 논문인 simGNN을 리뷰할 계획이다.  
